...
# The two scripts are identical except for which adapter module they build and
# the seed they pass, so the target dataset selects both.
...
class AnoVLAdapter(ModelAdapter):
    ...
    image the class's adapter is re-initialized from scratch and then trained for
    five AdamW steps at 1e-3 on an entropy objective computed from that image
    alone. Because the reset happens per image, no state carries from one image
    to the next, so this stays inside the matched clean-versus-adversarial
    protocol; but the reset, the 22-view ``aug`` draw and the adapter's own
    ``mask_aug`` all consume the global RNG, which would make a prediction depend
    on how many images preceded it. The adapter therefore reseeds before each
    image, so an image sees the same initialization and the same augmentations on
    the clean and the adversarial pass.
    ...
    def predict(
        ...
        for index in range(len(batch)):
            ...
            for _ in range(self.adapter_epochs):
                ...
                prediction = torch.softmax(adapted @ text_features, dim=-1)
